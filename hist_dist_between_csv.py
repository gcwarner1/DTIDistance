...
def plot_hist(FAmapHistogram, FAmapBinEdges, MDmapHistogram, MDmapBinEdges):
    # plot the histogram
    ...
    ax1.set_title('FA', fontsize=12)# (n='+str(len(FAxvals))+')',fontsize=14)
    ax2.set_title('MD', fontsize=12)
    ax1.bar(FAmapBinEdges[:-1], FAmapHistogram, width=FAmapBinEdges[1]-FAmapBinEdges[0], color='black')
    ax2.bar(MDmapBinEdges[:-1], MDmapHistogram, width=MDmapBinEdges[1]-MDmapBinEdges[0], color='black')
    for ax in [ax1, ax2]:
	ax.grid(False)
        ...
        for tick in ax.get_yticklabels():
                tick.set_fontsize(12)
    #plt.bar(mapBinEdges[:-1], mapHistogram, width=mapBinEdges[1]-mapBinEdges[0], color='black')
    #plt.grid(False)
    #plt.title(fileNamePath)
    plt.savefig('/space/jazz/1/users/gwarner/testOutFilePic.png')
...
def create_hist(listOfRuns, fileTypeList):
	...
	for run in listOfRuns:
        ...
        # ascertain if the file is NIfTI
			...
			print fileName
                        if fileName.endswith('_FA.nii.gz'):
				print 'FA found'
                                try:
                                    FAmapData = nib.load(fileNamePath)
                                except:
                                    ...
                                    FAdataOrig = FAmapData.get_data()
                                    FAdata = np.asfarray(FAdataOrig)  #convert from float32 to float64
                                    FAhdr = FAmapData.get_header()
                                    FAxmin=binRange[0][0]
                                    FAxmax=binRange[0][1]
                                    FAstep=stepList[0]
                                    FAmapHistogram, FAmapBinEdges = np.histogram(FAdata, bins=np.linspace(FAxmin, FAxmax, (FAxmax-FAxmin)/FAstep))
                                    # all of the background voxels are zero. Set their frequency to zero so plot scales better
                                    FAmapHistogram[0] = 0.0
                                    FAnormFactor = float(np.sum(FAmapHistogram))
                                    FAmapHistogramNorm = FAmapHistogram/FAnormFactor 
			elif fileName.endswith('_MD.nii.gz'):
				print 'MD found'
				try:
                                    MDmapData = nib.load(fileNamePath)
                                except:
                                    print fileNamePath, " is either not NIfTI or is unreadable."
                                    continue    #move on to next dataset
                                else:
                                    #print "...getting info from: ", fileNamePath
                                    MDdataOrig = MDmapData.get_data()
                                    MDdata = np.asfarray(MDdataOrig)  #convert from float32 to float64
                                    MDhdr = MDmapData.get_header()
                                    MDxmin=binRange[1][0]
                                    MDxmax=binRange[1][1]
                                    MDstep=stepList[1]
                                    MDmapHistogram, MDmapBinEdges = np.histogram(MDdata, bins=np.linspace(MDxmin, MDxmax, (MDxmax-MDxmin)/MDstep))
                                    # all of the background voxels are zero. Set their frequency to zero so plot scales better
                                    MDmapHistogram[0] = 0.0
				    print 'here'
                                    MDnormFactor = float(np.sum(MDmapHistogram))
                                    MDmapHistogramNorm = MDmapHistogram/MDnormFactor
                                    MDmapData = nib.load(fileNamePath)
		    plot_hist(FAmapHistogramNorm, FAmapBinEdges, MDmapHistogramNorm, MDmapBinEdges)
...
def main():
    ...
    #metricList = [hellinger]
    #metricListString = ['hellinger']
    #********************************************************
