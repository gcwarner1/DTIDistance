...
def sortData(data):
	...
	'''
	#info = {'SIEMENS':{'30':[]}, 'GE':{'30':[]}}
	info = {'6':[], '25':[]}
	for x in data:
		if x['Manufacturer'] == 'GE':
			if x['field'] == '1.5':
				if x['bvec'] in info.keys():
					info[x['bvec']].append(x['Path'])
	return info
#	for x in data:
#		if x['field'] == '1.5':
#		    if x['bval'] == "['1000']":
#			if x['bvec'] == '30' or x['bvec'] == '29' or x['bvec'] == '31':
#				info[x['Manufacturer']]['30'].append(x['Path'])
...
#			setOfRuns.add(z)
    ...
    for key in info: #key = 'GE'
	    outFile = '/space/jazz/1/users/gwarner/histograms/histdist_results_GE_'+key+'_Dirs_1.5T_1000_bval'
	    #arrayData = {'GE':{},'SIEMENS':{}}
            #for man in arrayData:
            #         faArray, faFileList, mdArray, mdFileList, fileCountList = create_hist_arrays(info[man]['30'], fileTypeList)
            ...
            #numPairs = (len(arrayData['GE'])*(len(arrayData['GE'])-1))/2
            #distListFA, distListMD = calc_hist_dist(arrayData, metricList)
            #lenMetricList = len(metricListString)
	    faArray, faFileList, mdArray, mdFileList, fileCountList = create_hist_arrays(info[key], fileTypeList)#info[key]['30'], fileTypeList)
